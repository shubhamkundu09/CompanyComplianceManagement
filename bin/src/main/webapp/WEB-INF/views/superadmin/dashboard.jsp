...
<div class="app-wrapper">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <a href="${pageContext.request.contextPath}/super-admin/dashboard" class="nav-item active">
        ...
        <div class="tabs-container">
            <div class="tabs">
                ...
                <button class="tab-btn" data-tab="overdue" onclick="switchTab('overdue')">
                    <i class="fas fa-exclamation-triangle" style="color: var(--danger);"></i> Overdue
                </button>
...
<script>
    ...
    async function loadStats() {
        try {
            ...
            if (companyData && companyData.success) {
                ...
                document.getElementById('companyCount').textContent = total;
            }
    ...
    function renderCompanies(companies) {
        ...
        tbody.innerHTML = companies.map(function(c) {
            ...
            return '<tr>' +
                ...
                '<td><span class="badge badge-info">' + (c.complianceCount || 0) + '</span></td>' +
                '<td style="font-size:12px;color:#64748b;">' + formatDate(c.createdAt) + '</td>' +
                '</tr>';
        }).join('');
    }
    ...
    function renderCategories(categories) {
        ...
        tbody.innerHTML = categories.map(function(c) {
            ...
            return '<tr>' +
                '<td><div style="display:flex;align-items:center;gap:8px;"><i class="fas fa-folder-open" style="color:var(--primary);"></i><span style="font-weight:500;">' + escapeHtml(c.name) + '</span></div></td>' +
                '<td style="font-size:12px;color:#64748b;">' + (c.description ? escapeHtml(c.description) : '—') + '</td>' +
                '<td><span class="badge ' + statusBadge + '">' + (c.isActive ? 'Active' : 'Inactive') + '</span></td>' +
                '<td style="text-align:center;">' + (c.companiesCount || 0) + '</td>' +
                '<td><button onclick="window.location.href=\'' + contextPath + '/super-admin/compliance/templates/' + c.id + '\'" class="btn btn-ghost" style="padding:4px 12px;font-size:11px;"><i class="fas fa-eye"></i> View</button></td>' +
                '</tr>';
        }).join('');
    }
    ...
    function renderOverdue(overdue) {
        ...
        tbody.innerHTML = overdue.map(function(c) {
            ...
            return '<tr>' +
                ...
                '<td>' + (c.subTemplateName ? escapeHtml(c.subTemplateName) : '—') + '</td>' +
                '<td style="font-size:12px;">' + formatDate(c.dueDate) + '</td>' +
                '<td><span class="badge badge-danger">' + overdueDays + ' days</span></td>' +
                '<td>' + (c.assignedByName || '—') + '</td>' +
                '</tr>';
        }).join('');
    }
    ...
    function renderCompleted(completed) {
        ...
        tbody.innerHTML = completed.slice(0, 50).map(function(c) {
            return '<tr>' +
                ...
                '<td>' + escapeHtml(c.templateName) + '</td>' +
                '<td>' + (c.subTemplateName ? escapeHtml(c.subTemplateName) : '—') + '</td>' +
                '<td>' + (c.assignedByName || 'System') + '</td>' +
                '<td style="font-size:12px;color:#64748b;">' + formatDateTime(c.assignedAt) + '</td>' +
                '</tr>';
        }).join('');
    }
