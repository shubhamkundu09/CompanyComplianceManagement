...
<div class="app-wrapper">
    ...
    <main class="main-content">
        ...
        <div class="breadcrumb">
            <a href="${pageContext.request.contextPath}/super-admin/compliance/templates">
...
<script>
    ...
    async function loadAssignedCompanies() {
        ...
        try {
            ...
            if (data && data.success) {
                assignedCompanies = data.data.content || [];
                console.log('Loaded ' + assignedCompanies.length + ' assigned companies');
  ...
  function updateUI() {
      ...
      // ================================================================
      // FIX: Hide "Add Sub-Compliance" button when parent is configured
      // ================================================================
      var addSubBtn = document.getElementById("addSubBtn");

      // If parent has configuration AND no sub-compliances exist, hide the Add Sub-Compliance button
      if (hasConfig && !hasSubs) {
          // Parent is configured with no sub-compliances - hide Add Sub-Compliance
          addSubBtn.style.display = "none";

          // Show the "Configure Parent" button as "Edit Configuration" since it's already configured
          document.getElementById("configureParentBtn").style.display = "none";
          document.getElementById("editParentConfigBtn").style.display = "inline-flex";
          ...
          // Show assign button if configured
      ...
      // If has sub-compliances OR no config, show Add Sub-Compliance button
      addSubBtn.style.display = "inline-flex";

      // Show/hide sections and buttons
      if (hasSubs) {
          // Has sub-compliances - show sub section, hide parent config section
          document.getElementById("configureParentBtn").style.display = "none";
          document.getElementById("editParentConfigBtn").style.display = "none";
          document.getElementById("subSection").style.display = "block";
          document.getElementById("configSection").style.display = "none";
      } else if (hasConfig) {
          // No sub-compliances but has parent config
          document.getElementById("configureParentBtn").style.display = "none";
          document.getElementById("editParentConfigBtn").style.display = "inline-flex";
          document.getElementById("subSection").style.display = "none";
          document.getElementById("configSection").style.display = "block";
      } else {
          // No sub-compliances and no config
          document.getElementById("configureParentBtn").style.display = "inline-flex";
          document.getElementById("editParentConfigBtn").style.display = "none";
          document.getElementById("subSection").style.display = "none";
          document.getElementById("configSection").style.display = "none";
      ...
      if (assignBtn) {
          if (hasAssignments) {
              // Already assigned - hide button
              assignBtn.style.display = "none";
          } else if (hasSubs) {
              // Has sub-compliances - show if at least one is configured
              var anyConfigured = subCompliances.some(function (s) {
                  return s.isConfigured === true;
              });
              if (anyConfigured) {
              ...
              // No sub-compliances but has parent config - show assign button
              assignBtn.style.display = "inline-flex";
              assignBtn.innerHTML = '<i class="fas fa-users"></i> Assign to Companies';
          } else {
              // No config and no sub-compliances - hide
              assignBtn.style.display = "none";
          }
      }

      // Update assigned companies count
      document.getElementById("assignedCompaniesCount").textContent =
          assignedCompanies.length;
      document.getElementById("companyCount").textContent =
          assignedCompanies.length;

      // Calculate configured vs pending companies
      var configured = 0,
          pending = 0;
      if (assignedCompanies.length > 0) {
          for (var i = 0; i < assignedCompanies.length; i++) {
              var hasConfigured = false;
              if (assignedCompanies[i].configured === true) {
                  hasConfigured = true;
              }
    ...
    function renderParentConfig() {
        ...
        html += '<div class="config-item"><div class="label">Frequency</div><div class="value">' + getFrequencyLabel(c.frequency) + '</div></div>';

        var dueDateDisplay = c.effectiveDueDate ? formatDate(c.effectiveDueDate) : (c.customDueDate ? formatDate(c.customDueDate) : '—');
        html += '<div class="config-item"><div class="label">Due Date</div><div class="value">' + dueDateDisplay + '</div></div>';

        html += '<div class="config-item"><div class="label">Reminder</div><div class="value">' + (c.reminderDaysBefore || 10) + ' days before</div></div>';
        html += '<div class="config-item"><div class="label">Repeat Reminder</div><div class="value">' + (c.repeatReminder !== false ? 'Yes' : 'No') + '</div></div>';

        if (c.instructions) {
            html += '<div class="config-item" style="grid-column:1/-1;"><div class="label">Instructions</div><div class="value" style="white-space:pre-line;font-size:13px;">' + escapeHtml(c.instructions) + '</div></div>';
        }
        if (c.documentRequired) {
            html += '<div class="config-item"><div class="label">Required Documents</div><div class="value"><i class="fas fa-file-alt" style="color:var(--primary);"></i> ' + escapeHtml(c.documentRequired) + '</div></div>';
        }
        if (c.externalLink) {
            html += '<div class="config-item"><div class="label">External Portal</div><div class="value"><a href="' + escapeHtml(c.externalLink) + '" target="_blank"><i class="fas fa-external-link-alt"></i> Open Portal</a></div></div>';
        }
    ...
    function renderSubCompliances() {
        ...
        if (!subCompliances.length) {
            container.innerHTML = '<div class="empty-state"><i class="fas fa-plus-circle"></i> No sub-compliances created yet. Click "Add Sub-Compliance" to create one.</div>';
            return;
        }
        ...
        for (var i = 0; i < subCompliances.length; i++) {
            ...
            if (isConfigured && configDetails) {
                ...
                var due = configDetails.effectiveDueDate ? formatDate(configDetails.effectiveDueDate) :
                          (configDetails.customDueDate ? formatDate(configDetails.customDueDate) : '—');
                var reminder = (configDetails.reminderDaysBefore || 10) + ' days before';
                ...
                configHtml = '<div class="config-details">' +
                    (freq !== '—' ? '<span><i class="fas fa-redo" style="color:var(--primary);"></i> ' + freq + '</span>' : '') +
                    (due !== '—' ? '<span class="' + dueClass + '"><i class="fas fa-calendar-alt" style="color:var(--primary);"></i> Due: ' + due + '</span>' : '') +
                    (reminder !== '—' ? '<span><i class="fas fa-bell" style="color:var(--primary);"></i> Reminder: ' + reminder + '</span>' : '') +
                    '</div>';

                if (configDetails.instructions) {
                    configHtml += '<div style="margin-top:4px;font-size:11px;color:var(--gray-500);width:78%;">' +
                        '<i class="fas fa-info-circle" style="color:var(--primary);"></i> ' + escapeHtml(configDetails.instructions) +
                        '</div>';
                }
            } else {
                configHtml = '<div style="margin-top:6px;font-size:11px;color:var(--warning);">' +
   ...
   function renderAssignedCompanies() {
    ...
    for (var i = 0; i < uniqueList.length; i++) {
        var hasConfigured = uniqueList[i].assignments.some(function(a) { return a.configured === true; });
        if (hasConfigured) configured++;
        else pending++;
    }
    document.getElementById('configuredCount').textContent = configured;
    ...
    if (!uniqueList.length) {
        container.innerHTML = '<div class="empty-state"><i class="fas fa-building"></i> No companies assigned yet. Configure the compliance to auto-assign to all active companies.</div>';
        return;
    }
    ...
    for (var i = 0; i < uniqueList.length; i++) {
        ...
        html += '<div class="company-card" style="border-left:3px solid ' +
            (overallStatus === 'COMPLETED' ? 'var(--success)' : overallStatus === 'OVERDUE' ? 'var(--danger)' : 'var(--primary)') + ';">' +
        ...
        for (var j = 0; j < comp.assignments.length; j++) {
            ...
            html += '<div class="sub-item" style="border-left-color:' +
                (subStatus === 'COMPLETED' ? 'var(--success)' : subStatus === 'OVERDUE' ? 'var(--danger)' : 'var(--primary)') + ';">' +
                '<div style="display:flex;flex-direction:column;gap:2px;flex:1;min-width:0;">' +
                '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">' +
                '<span class="sub-name">' + escapeHtml(subName) + '</span>' +
                '<span class="badge ' + (isConfigured ? 'badge-active' : 'badge-pending') + '" style="font-size:9px;padding:1px 8px;">' + (isConfigured ? 'Configured' : 'Pending') + '</span>' +
                '</div>' +
                ...
                '</div>' +
                '</div>' +
                '<span class="sub-status ' + subStatusClass + '" style="font-size:9px;padding:2px 10px;flex-shrink:0;">' + subStatusLabel + '</span>' +
                '</div>';
        }
    ...
    async function removeCompany(companyId, companyName) {
        if (!confirm('⚠️ Are you sure you want to remove "' + companyName + '" from this compliance category?\n\n' +
            'This will deactivate the compliance for this company. The configuration will be preserved.')) {
            return;
        }

        try {
            // Get all assignments for this company and template
            var data = await api('/api/super-admin/compliance/assignments?page=0&size=100&companyId=' + companyId + '&templateId=' + TEMPLATE_ID);

            if (data && data.success) {
                var assignments = data.data.content || [];

                if (assignments.length === 0) {
                    toast('No assignments found for this company', 'error');
                    return;
                }
                ...
                for (var i = 0; i < assignments.length; i++) {
                    var result = await api('/api/super-admin/compliance/assignments/' + assignments[i].id, {
                        method: 'DELETE'
                    });
                ...
                if (successCount > 0) {
                    toast('Removed ' + successCount + ' assignment(s) for "' + companyName + '"' +
                        (errorCount > 0 ? ' (' + errorCount + ' failed)' : ''), 'success');
                    await loadAssignedCompanies();
                    updateUI();
                } else {
                    toast('Failed to remove company. Please try again.', 'error');
                }
            } else {
    ...
    function openConfigModal(id, type) {
        ...
        if (type === 'template' && parentConfig) {
            var c = parentConfig;
            document.getElementById('configFrequency').value = c.frequency || 'ONE_TIME';
            ...
            showConfigSections(c.frequency || 'ONE_TIME');
        } else {
