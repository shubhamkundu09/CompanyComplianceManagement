            ...
            <form id="templateForm" onsubmit="return false;">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <div style="font-size:11px;color:var(--gray-500);margin-top:4px;">
                        <i class="fas fa-info-circle"></i> Priority 0 is highest. Categories with lower numbers appear first.
                    </div>
                </div>
...
<script>
    ...
    async function loadTemplates() {
        ...
        // ===== FIX: Add sorting to the API request =====
        var url = '/api/super-admin/compliance/templates?page=' + currentPage + '&size=' + pageSize + '&sortBy=priority&sortDir=asc';
        var data = await api(url);

        if (data && data.success) {
            var pageData = data.data;
            allTemplates = pageData.content || [];

            // Enrich with additional data
            for (var i = 0; i < allTemplates.length; i++) {
                var template = allTemplates[i];
                await enrichTemplateData(template);
            }

            // Apply filters (which now sorts by priority)
            var filtered = applyFilters(allTemplates, statusFilter, configFilter, search);
            ...
            // ===== FIX: Use the actual page data for pagination =====
            // The filtered array contains all items, but we need to paginate based on the API response
            // Use the pageData from the API response for pagination
            var totalElements = pageData.totalElements;
            var totalPages = pageData.totalPages;
            var currentPageNum = pageData.number;

            renderTemplates(allTemplates); // Show the current page's templates
            renderPagination(totalPages, currentPageNum, totalElements);
    ...
    function applyFilters(templates, statusFilter, configFilter, search) {
        var filtered = templates.slice();

        if (statusFilter !== "all") {
            var showActive = statusFilter === "true";
            filtered = filtered.filter(function (t) {
                return t.isActive === showActive;
            });
        }
        ...
        // ===== FIX: Sort by priority first (lower number = higher priority) =====
        filtered.sort(function(a, b) {
            // First sort by priority (lower number first)
    ...
    function updateStats(templates) {
        ...
        var configured = templates.filter(function(t) { return t.isConfigured; }).length;
    ...
    function renderTemplates(templates) {
        ...
        if (!templates.length) {
            grid.innerHTML =
                '<div class="empty-state-container">No categories found</div>';
            return;
        }

        var html = "";
        for (var i = 0; i < templates.length; i++) {
            // Ensure assignedCompaniesCount is set (default to 0 if not)
            if (templates[i].assignedCompaniesCount === undefined) {
                templates[i].assignedCompaniesCount = 0;
            }
      ...
      function buildTemplateCard(t) {
          ...
          var isConfigured = t.isConfigured || false;
          var hasSubs = t.hasSubCompliances || false;
          var subCount = t.subCount || 0;
          var assignedCount = t.assignedCompaniesCount || 0;
          var allConfigured = t.allConfigured || false;
          var priority = t.priority || 0;  // <-- ADD THIS LINE
          ...
          if (hasSubs) {
              if (allConfigured) {
                  configBadge = '<span class="badge badge-configured"><i class="fas fa-check"></i> All Configured</span>';
              } else if (isConfigured) {
                  configBadge = '<span class="badge badge-pending-config" style="background:rgba(245,158,11,0.15);color:var(--warning);"><i class="fas fa-clock"></i> Partial</span>';
              } else {
                  configBadge = '<span class="badge badge-pending-config"><i class="fas fa-clock"></i> Pending</span>';
              }
          } else {
              if (isConfigured) {
                  configBadge = '<span class="badge badge-configured"><i class="fas fa-check"></i> Configured</span>';
              } else {
                  configBadge = '<span class="badge badge-pending-config"><i class="fas fa-clock"></i> Pending</span>';
              }
          }

          var subBadge = hasSubs
          ...
          // <-- ADD THIS PRIORITY BADGE
          var priorityBadge = '<span class="badge badge-info" style="background:rgba(79,70,229,0.1);color:var(--primary);border:1px solid rgba(79,70,229,0.2);"><i class="fas fa-sort-numeric-down"></i> Priority: ' + priority + '</span>';
          ...
          if (hasSubs) {
              if (allConfigured) {
                  statusText = "All Configured";
              } else if (isConfigured) {
                  statusText = "Partial Config";
              } else {
                  statusText = "Pending";
              }
          } else {
          ...
          var toggleIcon = isActive ? "fa-pause-circle" : "fa-play-circle";
          var toggleTitle = isActive ? "Deactivate" : "Activate";
          var toggleColor = isActive ? "btn-warning" : "btn-success";
          ...
          var html =
              ...
              subBadge +
              assignedBadge +
              priorityBadge +  // <-- ADD THIS LINE
              "</div>" +
              "</div>" +
              '<div class="card-body">' +
              '<div class="description">' + (escapeHtml(t.description) || "No description provided.") +
              "</div>" +
              "</div>" +
              '<div class="card-stats">' +
              '<div class="stat-item">' +
              '<div class="number configured-color">' + assignedCount + "</div>" +
              '<div class="label">Assigned Companies</div>' +
              "</div>" +
              '<div class="stat-item">' +
              '<div class="number ' + (isConfigured ? "active-color" : "pending-color") + '">' +
              (allConfigured ? "✓ All" : isConfigured ? "⏳ Part" : "⏳") +
              "</div>" +
              '<div class="label">' + statusText + "</div>" +
              "</div>" +
              '<div class="stat-item">' +
              '<div class="number ' + (hasSubs ? "active-color" : "pending-color") + '">' + subCount + "</div>" +
              '<div class="label">Sub-Compliances</div>' +
              "</div>" +
              "</div>" +
              '<div class="card-footer">' +
              '<a href="' + viewUrl + '" class="btn btn-primary" title="Manage Sub-Compliances">' +
              '<i class="fas fa-layer-group"></i> Manage</a>' +
              '<button onclick="viewComplianceHistory(' + t.id + ')" class="btn btn-ghost btn-icon" title="View History">' +
              '<i class="fas fa-history"></i></button>' +
              // <-- ADD THIS EDIT PRIORITY BUTTON

              '<button onclick="editTemplate(' + t.id + ')" class="btn btn-ghost btn-icon" title="Edit Category">' +
              '<i class="fas fa-edit"></i></button>' +
              '<button onclick="toggleStatus(' + t.id + ", " + isActive + ')" class="btn ' + toggleColor + ' btn-icon" title="' + toggleTitle + '">' +
              '<i class="fas ' + toggleIcon + '"></i></button>' +
              '<button onclick="openDeleteModal(' + t.id + ", '" + escapeHtml(t.name) + "', " + assignedCount + ')" class="btn btn-danger btn-icon" title="Delete">' +
