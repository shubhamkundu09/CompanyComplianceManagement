...
<script>
   ...
   async function loadParentDetails() {
       ...
       try {
           ...
           if (data && data.success) {
               ...
               // Map sub‑compliances, including completion details
               var subCompliances = subCompliancesData.map(function (s) {
                   var status = s.status || "PENDING";
                   var dueDate = s.dueDate || s.effectiveDueDate;
                   if (dueDate) {
                       var due = new Date(dueDate);
                       due.setHours(0, 0, 0, 0);
               ...
               if (subCompliances.length > 0) {
                   if (subCompliances.every(function (s) { return s.status === "COMPLETED"; })) {
                       overallStatus = "COMPLETED";
                   } else if (subCompliances.some(function (s) { return s.status === "OVERDUE"; })) {
                       overallStatus = "OVERDUE";
                   } else if (subCompliances.some(function (s) { return s.status === "IN_PROGRESS"; })) {
                   ...
                   var parentDueDate = foundParentEntry ? foundParentEntry.dueDate : targetEntry.dueDate;
                   if (parentDueDate) {
                       var due = new Date(parentDueDate);
                       due.setHours(0, 0, 0, 0);
                       if (due < today && overallStatus !== "COMPLETED") {
                           overallStatus = "OVERDUE";
                       }
                   }
               }
               ...
               if (parentData.hasSubCompliances) {
                   ...
                   var hasPending = subCompliances.some(function (s) { return s.status !== 'COMPLETED'; });
   ...
   function renderParentConfig() {
    ...
    function openCompletionDetailsModal(index) {
        ...
        var completedBy = s.completedByName || 'Company Admin';
        var completedAt = s.completedAt;
        var submissionReference = s.submissionReference;
        var submissionDocumentUrl = s.submissionDocumentUrl;

        html += '<div><div style="font-size:10px;color:var(--gray-500);text-transform:uppercase;letter-spacing:0.4px;font-weight:600;">Completed By</div>' +
            '<div style="font-size:14px;font-weight:600;color:var(--gray-900);margin-top:3px;">' + escapeHtml(completedBy) + '</div></div>';

        if (completedAt) {
            html += '<div><div style="font-size:10px;color:var(--gray-500);text-transform:uppercase;letter-spacing:0.4px;font-weight:600;">Completed On</div><div style="font-size:14px;font-weight:600;color:var(--gray-900);margin-top:3px;">' + formatDateTime(completedAt) + '</div></div>';
        }

        if (submissionReference) {
            html += '<div style="grid-column:1/-1;"><div style="font-size:10px;color:var(--gray-500);text-transform:uppercase;letter-spacing:0.4px;font-weight:600;">Reference Number</div><div style="font-size:14px;font-weight:600;color:var(--gray-900);margin-top:3px;font-family:monospace;">' + escapeHtml(submissionReference) + '</div></div>';
        }
        ...
        if (!submissionReference && !submissionDocumentUrl) {
            html += '<div style="grid-column:1/-1;color:var(--gray-500);font-size:13px;padding:8px 0;">No submission details were recorded.</div>';
        }
    ...
    function renderCompletionDetails(s, history) {
        var html = '<div style="background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.15);border-radius:10px;padding:20px;">' +
            '<div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">' +
            '<i class="fas fa-check-circle" style="color:var(--success);font-size:24px;"></i>' +
            '<span style="font-weight:700;font-size:17px;color:var(--success);">Completed Successfully</span>' +
        ...
        // Prefer history data, fallback to the local object
        var completedBy = (history && history.employeeName) ? history.employeeName : (s.completedByName || 'Company Admin');
        var completedAt = (history && history.completedAt) ? history.completedAt : s.completedAt;
        var submissionReference = (history && history.submissionReference) ? history.submissionReference : s.submissionReference;
        var submissionDocumentUrl = (history && history.submissionDocumentUrl) ? history.submissionDocumentUrl : s.submissionDocumentUrl;
